...
import org.sakaiproject.scorm.ui.console.pages.PackageConfigurationPage.DisplayNamePropertyModel;
...
public class UploadPage extends ConsoleBasePage implements ScormConstants {
	...
	public UploadPage(PageParameters params) {
		...
		boolean canUpload = lms.canUpload(context);
		
		if(canUpload) {
			add(new FileUploadForm("uploadForm"));			
			add(new Label( "upload.not.Permission",new ResourceModel( "page.notAllowed" ) ).setVisible(false));
		} else {
			add(new FileUploadForm("uploadForm").setVisible(false));
			add(new Label( "upload.not.Permission",new ResourceModel( "page.notAllowed" ) ));
		}
	}
