...
public class ScormCHH extends FastZipCHH implements ScormConstants {
	...
	protected VirtualFileSystem buildVirtualFileSystem(ContentEntity realParent) {
		...
		log.debug("Invoke buildVirtualFileSystem in scorm-content-impl");
	...
	private InputStream getManifestAsStream(String contentPackageId) {
		...
		log.debug("Invoke getManifestAsStream in scorm-content-impl. contentPkgID : "+contentPackageId);
		...
		try {
			...
			log.debug("Invoke getManifestAsStream .. Param  : "+path);
			if (resource != null) {
