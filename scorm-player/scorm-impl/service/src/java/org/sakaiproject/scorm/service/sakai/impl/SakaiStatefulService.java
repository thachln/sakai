...
public abstract class SakaiStatefulService implements LearningManagementSystem, ScormConstants {
	...
	protected boolean canLaunchAttemptInternal(ContentPackage contentPackage, long attemptNumber) {
		...
		if (canModify(context)) {
			...
			if (status != ScormConstants.CONTENT_PACKAGE_STATUS_OPEN 
	...
	// Hoctdy edit can modify start
	public boolean canModify(String context) {
		return canConfigure(context) || canDelete(context)|| canUpload(context);
	}
	// Hoctdy edit can modify end
	...
	public String getRoleLearner(String leranerId, String siteId) {
		String result = "";
		List<Site> siteLst = siteService().getUserSites(true, leranerId);
		for(Site tmp: siteLst){
			if(tmp.getId().equals(siteId)){
				Role role = tmp.getUserRole(leranerId);				
				return role.getId();
			}
		}
		return result;
