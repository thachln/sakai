...
@Slf4j
public class GradebookExternalAssessmentServiceImpl extends BaseHibernateManager
	...
	// Mapping of providers to their getAllExternalAssignments(String
	// gradebookUid) methods,
	// used to allow the method to be called on providers not declaring the
	// Compat interface.
	// This is to allow the same code to be used on 2.9 and beyond, where the
	// secondary interface
	// may be removed, without build profiles.
	private ConcurrentHashMap<ExternalAssignmentProvider, Method> providerMethods = new ConcurrentHashMap<ExternalAssignmentProvider, Method>();

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.sakaiproject.service.gradebook.shared.
	 * GradebookExternalAssessmentService#registerExternalAssignmentProvider(org
	 * .sakaiproject.service.gradebook.shared.ExternalAssignmentProvider)
	 */
	@Override
	public void registerExternalAssignmentProvider(ExternalAssignmentProvider provider) {
		if (provider == null) {
			...
			// Try to duck-type the provider so it doesn't have to declare the
			// Compat interface.
			// TODO: Remove this handling once the Compat interface has been
			// merged or the issue is otherwise resolved.
			if (!(provider instanceof ExternalAssignmentProviderCompat)) {
				try {
					...
					log.warn("ExternalAssignmentProvider [" + provider.getAppKey() + " / "
	 ...
	 * 
	 * @see org.sakaiproject.service.gradebook.shared.
	 * GradebookExternalAssessmentService#unregisterExternalAssignmentProvider(
	 * java.lang.String)
	 */
	@Override
	...
	/**
	 * Property in sakai.properties used to allow this service to update scores
	 * in the db every time the update method is called. By default, scores are
	 * only updated if the score is different than what is currently in the db.
	 */
	public static final String UPDATE_SAME_SCORE_PROP = "gradebook.externalAssessments.updateSameScore";

	@Override
	public synchronized void addExternalAssessment(final String gradebookUid, final String externalId,
			final String externalUrl, final String title, final double points, final Date dueDate,
			final String externalServiceDescription)
		...
		// name cannot contain these chars as they are reserved for special
		// columns in import/export
		...
		getHibernateTemplate().execute(session -> {
			...
			Number externalIdConflicts = (Number) session.createCriteria(GradebookAssignment.class)
					.createAlias("gradebook", "g").add(Restrictions.eq("externalId", externalId))
					.add(Restrictions.eq("g.uid", gradebookUid)).setProjection(Projections.rowCount()).uniqueResult();

			if (externalIdConflicts.intValue() > 0) {
				throw new ConflictingExternalIdException("An external assessment with ID=" + externalId
						+ " already exists in gradebook uid=" + gradebookUid);
			}
		...
		log.info("External assessment added to gradebookUid={}, externalId={} by userUid={} from externalApp={}",
				gradebookUid, externalId, getUserUid(), externalServiceDescription);
	}

	/**
	 * @see org.sakaiproject.service.gradebook.shared.GradebookService#updateExternalAssessment(java.lang.String,
	...
	public void updateExternalAssessment(final String gradebookUid, final String externalId, final String externalUrl,
		...
		HibernateCallback<?> hc = new HibernateCallback<Object>() {
			...
			public Object doInHibernate(Session session) throws HibernateException {
				...
				session.update(asn);
				log.info("External assessment updated in gradebookUid={}, externalId={} by userUid={}", gradebookUid,
						externalId, getUserUid());
				return null;
			}
		};
	...
	/**
	 * @see org.sakaiproject.service.gradebook.shared.GradebookService#removeExternalAssessment(java.lang.String,
	 *      java.lang.String)
	 */
	@Override
	public void removeExternalAssessment(final String gradebookUid, final String externalId)
			throws GradebookNotFoundException, AssessmentNotFoundException {
		// Get the external assignment
		final GradebookAssignment asn = getExternalAssignment(gradebookUid, externalId);
		if (asn == null) {
			throw new AssessmentNotFoundException(
					"There is no external assessment id=" + externalId + " in gradebook uid=" + gradebookUid);
		}
		...
		hibTempl.execute((HibernateCallback<?>) session -> {
			int numDeleted = session.createQuery("delete GradingEvent where gradableObject=:go").setParameter("go", asn)
					.executeUpdate();
			log.debug("Deleted {} records from gb_grading_event_t", numDeleted);

			numDeleted = session.createQuery("delete AssignmentGradeRecord where gradableObject=:go")
					.setParameter("go", asn).executeUpdate();
			log.info("Deleted {} externally defined scores", numDeleted);

			numDeleted = session.createQuery("delete Comment where gradableObject=:go").setParameter("go", asn)
					.executeUpdate();
		...
		log.info("External assessment removed from gradebookUid={}, externalId={} by userUid={}", gradebookUid,
				externalId, getUserUid());
	}
	...
	private GradebookAssignment getExternalAssignment(final String gradebookUid, final String externalId)
		...
		HibernateCallback<GradebookAssignment> hc = session -> (GradebookAssignment) session
				.createQuery(
						"from GradebookAssignment as asn where asn.gradebook = :gradebook and asn.externalId = :externalid")
				.setEntity("gradebook", gradebook).setString("externalid", externalId).uniqueResult();
	...
	public void updateExternalAssessmentComments(final String gradebookUid, final String externalId,
			final Map<String, String> studentUidsToComments)
			throws GradebookNotFoundException, AssessmentNotFoundException {
		...
		getHibernateTemplate().execute(session -> {
			...
			List<AssignmentGradeRecord> existingScores = session.createCriteria(AssignmentGradeRecord.class)
					...
					.add(HibernateCriterionUtils.CriterionInRestrictionSplitter("studentId", studentIds)).list();
			...
			for (AssignmentGradeRecord agr : existingScores) {
				...
				// has changed or property has been set forcing a db update
				// every time.
				...
				if (alwaysUpdate || (newComment != null && !newComment.equals(oldComment))
						|| (newComment == null && oldComment != null)) {
					changedStudents.add(studentUid);
					setAssignmentScoreComment(gradebookUid, asn.getId(), studentUid, newComment);
				}
			}

			log.debug("updateExternalAssessmentScores sent {} records, actually changed {}", studentIds.size(),
	...
	public void updateExternalAssessmentScores(final String gradebookUid, final String externalId,
			final Map<String, Double> studentUidsToScores)
			throws GradebookNotFoundException, AssessmentNotFoundException {
	...
	public void updateExternalAssessmentScoresString(final String gradebookUid, final String externalId,
			final Map<String, String> studentUidsToScores)
			throws GradebookNotFoundException, AssessmentNotFoundException {
		...
		getHibernateTemplate().execute(session -> {
			...
			for (AssignmentGradeRecord agr : existingScores) {
				...
				// TODO: for ungraded items, needs to set ungraded-grades
				// later...
				Double oldPointsEarned = agr.getPointsEarned();
				// Double newPointsEarned =
				// (Double)studentUidsToScores.get(studentUid);
				String newPointsEarnedString = studentUidsToScores.get(studentUid);
				Double newPointsEarned = (newPointsEarnedString == null) ? null
						: convertStringToDouble(newPointsEarnedString);
				if (alwaysUpdate || (newPointsEarned != null && !newPointsEarned.equals(oldPointsEarned))
						|| (newPointsEarned == null && oldPointsEarned != null)) {
					agr.setDateRecorded(now);
					agr.setGraderId(graderId);
			...
			for (String studentUid : previouslyUnscoredStudents) {
				...
				if (newPointsEarned != null) {
					...
					session.save(agr);
					changedStudents.add(studentUid);
					postUpdateGradeEvent(gradebookUid, assignment.getName(), studentUid,
							convertStringToDouble(newPointsEarned));
				}
			}
	...
	public boolean isAssignmentDefined(final String gradebookUid, final String assignmentName)
			throws GradebookNotFoundException {
		GradebookAssignment assignment = (GradebookAssignment) getHibernateTemplate()
	 ...
	 * 
	 * @see org.sakaiproject.service.gradebook.shared.
	 * GradebookExternalAssessmentService#isExternalAssignmentDefined(java.lang.
	 ...
	 * 
	 * @see org.sakaiproject.service.gradebook.shared.
	 * GradebookExternalAssessmentService#isExternalAssignmentGrouped(java.lang.
	 * String, java.lang.String)
	 */
	@Override
	public boolean isExternalAssignmentGrouped(String gradebookUid, String externalId)
		...
		// SAK-19668
		final GradebookAssignment assignment = getExternalAssignment(gradebookUid, externalId);
		// If we check all available providers for an existing, externally
		// maintained assignment
		// and none manage it, return false since grouping is the outlier case
		// and all items
		...
		if (assignment == null) {
			result = false;
			log.info("No assignment found for external assignment check: gradebookUid=" + gradebookUid + ", externalId="
					+ externalId);
		} else {
	 ...
	 * 
	 * @see org.sakaiproject.service.gradebook.shared.
	 * GradebookExternalAssessmentService#isExternalAssignmentVisible(java.lang.
	...
	public boolean isExternalAssignmentVisible(String gradebookUid, String externalId, String userId)
		...
		// If we check all available providers for an existing, externally
		// maintained assignment
		// and none manage it, assume that it should be visible. This matches
		// the pre-2.9 behavior
		// when a provider is not implemented to handle the assignment. Also,
		// any provider that
	...
	public Map<String, String> getExternalAssignmentsForCurrentUser(String gradebookUid)
		...
		// We include those items that the gradebook has marked as externally
		// maintained, but no provider has
		// identified as items under its authority. This maintains the behavior
		// prior to the grouping support
		// introduced for the 2.9 release (SAK-11485 and SAK-19688), where a
		// tool that does not have a provider
		// implemented does not have its items filtered for student views and
		// grading.
		List<org.sakaiproject.service.gradebook.shared.Assignment> gbAssignments = getGradebookService()
		...
		for (org.sakaiproject.service.gradebook.shared.Assignment assignment : gbAssignments) {
			...
			if (assignment.isExternallyMaintained() && !providedAssignments.contains(id)
					&& !visibleAssignments.containsKey(id)) {
				log.debug("External assignment in gradebook [{}] is not handled by a provider; ID: {}", gradebookUid,
						id);
	...
	protected Set<String> getProvidedExternalAssignments(String gradebookUid) {
		...
		for (ExternalAssignmentProvider provider : getExternalAssignmentProviders().values()) {
			// TODO: This is a temporary cast; if this method proves to be the
			// right fit
			// and perform well enough, it will be moved to the regular
			// interface.
			if (provider instanceof ExternalAssignmentProviderCompat) {
				allAssignments
						.addAll(((ExternalAssignmentProviderCompat) provider).getAllExternalAssignments(gradebookUid));
	...
	public Map<String, List<String>> getVisibleExternalAssignments(String gradebookUid, Collection<String> studentIds)
		...
		for (ExternalAssignmentProvider provider : getExternalAssignmentProviders().values()) {
			// SAK-24407 - Some tools modify this set so we can't pass it. I
			// considered making it an unmodifableCollection but that would
		...
		// SAK-23733 - This covers a tricky case where items that the gradebook
		// thinks are external
		// but are not reported by any provider should be included for everyone.
		// This is
		// to accommodate tools that use the external assessment mechanisms but
		// have not
		// implemented an ExternalAssignmentProvider.
		List<org.sakaiproject.service.gradebook.shared.Assignment> allAssignments = getGradebookService()
				.getViewableAssignmentsForCurrentUser(gradebookUid);
	...
	public void setExternalAssessmentToGradebookAssignment(final String gradebookUid, final String externalId) {
		...
		getHibernateTemplate().execute((HibernateCallback<?>) session -> {
			...
			log.info("Externally-managed assignment {} moved to Gradebook management in gradebookUid={} by userUid={}",
					externalId, gradebookUid, getUserUid());
			return null;
		});
	}
	...
	public void addExternalAssessment(final String gradebookUid, final String externalId, final String externalUrl,
			...
			final Boolean ungraded) throws GradebookNotFoundException, ConflictingAssignmentNameException,
			ConflictingExternalIdException, AssignmentHasIllegalPointsException {
		addExternalAssessment(gradebookUid, externalId, externalUrl, title, points, dueDate, externalServiceDescription,
				ungraded, null);
	}
	...
	public synchronized void addExternalAssessment(final String gradebookUid, final String externalId,
			final String externalUrl, final String title, final Double points, final Date dueDate,
			final String externalServiceDescription, final Boolean ungraded, final Long categoryId)
			...
			if (categoryId != null) {
				...
				if (persistedCategory == null || persistedCategory.isRemoved()
					...
					throw new InvalidCategoryException(
							"The category with id " + categoryId + " is not valid for gradebook " + gradebook.getUid());
				}
			}
	...
	@Override
	public void updateExternalAssessment(final String gradebookUid, final String externalId, final String externalUrl,
			final String title, final Double points, final Date dueDate, final Boolean ungraded)
	...
	@Override
	public void updateExternalAssessmentComment(final String gradebookUid, final String externalId,
			final String studentUid, final String comment)
			throws GradebookNotFoundException, AssessmentNotFoundException {
		final GradebookAssignment asn = getExternalAssignment(gradebookUid, externalId);
		...
		HibernateCallback<?> hc = session -> {
			...
			// score has actually changed or property has been set forcing a db
			// update every time.
			boolean alwaysUpdate = ServerConfigurationService.getBoolean(UPDATE_SAME_SCORE_PROP, false);
		...
		log.debug("END: Update 1 score for gradebookUid={}, external assessment={} from {}", gradebookUid, externalId,
				asn.getExternalAppName());
		log.debug("External assessment comment updated in gradebookUid={}, externalId={} by userUid={}, new score={}",
				gradebookUid, externalId, getUserUid(), comment);
	}
	...
	public void updateExternalAssessmentScore(final String gradebookUid, final String externalId,
		...
		HibernateCallback<?> hc = session -> {
			...
			if (alwaysUpdate || (newPointsEarned != null && !newPointsEarned.equals(oldPointsEarned))
				...
				log.debug("About to save AssignmentGradeRecord id={}, version={}, studenttId={}, pointsEarned={}",
						agr.getId(), agr.getVersion(), agr.getStudentId(), agr.getPointsEarned());
		...
		log.debug("END: Update 1 score for gradebookUid={}, external assessment={} from {}", gradebookUid, externalId,
				asn.getExternalAppName());
		log.debug("External assessment score updated in gradebookUid={}, externalId={} by userUid={}, new score={}",
				gradebookUid, externalId, getUserUid(), points);
	}

	/**
	 * Implement Updates an external score as learned progress for an external
	 * lessen in the gradebook
	 * 
	 * @param gradebookUid
	 ...
	 * @param points
	 *            The number of points earned on this assessment, or null if a
	 ...
	 */
	public void updateExternalAssessmentScore(final String gradebookUid, final String externalId,
			final String studentUid, final String points, final String percents, final String completionStatus)
			throws GradebookNotFoundException, AssessmentNotFoundException {
		final GradebookAssignment asn = getExternalAssignment(gradebookUid, externalId);
		...
		if (logData.isDebugEnabled())
			logData.debug("BEGIN: Update 1 score for gradebookUid=" + gradebookUid + ", external assessment="
					+ externalId + " from " + asn.getExternalAppName());
		...
		HibernateCallback hc = new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException {
				...
				// Try to reduce data contention by only updating when the
				// score has actually changed or property has been set forcing a
				// db update every time.
				boolean alwaysUpdate = ServerConfigurationService.getBoolean(UPDATE_SAME_SCORE_PROP, false);
				...
				if (alwaysUpdate || (newPointsEarned != null && !newPointsEarned.equals(oldPointsEarned))
					...
					if (log.isDebugEnabled())
						log.debug("About to save AssignmentGradeRecord id=" + agr.getId() + ", version="
								+ agr.getVersion() + ", studenttId=" + agr.getStudentId() + ", pointsEarned="
								+ agr.getPointsEarned());

					Double percentEarned = (percents == null) ? null : convertStringToDouble(percents);
					agr.setPercentEarned(percentEarned);
					// HocTDY.Added.20180704.START
					agr.setCompletionStatus(completionStatus);
					// HocTDY.Added.20180704.END
					session.saveOrUpdate(agr);
					...
					if (log.isDebugEnabled())
						log.debug(
								"Ignoring updateExternalAssessmentScore, since the new points value is the same as the old");
				}
				return null;
			}
		};
		getHibernateTemplate().execute(hc);
		if (logData.isDebugEnabled())
			logData.debug("END: Update 1 score for gradebookUid=" + gradebookUid + ", external assessment=" + externalId
					+ " from " + asn.getExternalAppName());
		if (log.isDebugEnabled())
			log.debug("External assessment score updated in gradebookUid=" + gradebookUid + ", externalId=" + externalId
					+ " by userUid=" + getUserUid() + ", new score=" + points);
	}

	private void postUpdateGradeEvent(String gradebookUid, String assignmentName, String studentUid,
			Double pointsEarned) {
		if (eventTrackingService != null) {
			eventTrackingService.postEvent("gradebook.updateItemScore", "/gradebook/" + gradebookUid + "/"
					+ assignmentName + "/" + studentUid + "/" + pointsEarned + "/student");
		}
	}
	...
	private Double convertStringToDouble(final String s) {
		...
		if (doubleAsString != null && !"".equals(doubleAsString)) {
			try {
				// check if grade uses a comma as separator because of number
				// format and change to a comma y the external app sends a point
				// as separator
