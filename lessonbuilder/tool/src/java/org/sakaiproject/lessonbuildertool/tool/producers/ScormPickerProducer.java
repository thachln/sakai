...
public class ScormPickerProducer implements ViewComponentProducer, NavigationCaseReporter, ViewParamsReporter {
	private static final Logger log = LoggerFactory.getLogger(ScormPickerProducer.class);
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams, ComponentChecker checker) {
		...
		if (simplePageBean.canEditPage()) {
			...
			List<LessonEntity> plist = scormEntity.getEntitiesInSite();
			...
			if (plist == null || plist.size() < 1) {
				UIOutput.make(tofill, "error-div");
				UIOutput.make(tofill, "error", messageLocator.getMessage("simplepage.no_content_packages"));
				UICommand.make(tofill, "cancel", messageLocator.getMessage("simplepage.cancel"),
						"#{simplePageBean.cancel}");
				return;
			}
			...
			UISelect select = UISelect.make(form, "scorm-span", values.toArray(new String[1]),
					"#{simplePageBean.selectedScorm}", currentItem);
