 ...
 */
@Slf4j
public class ShowPageProducer
		implements ViewComponentProducer, DefaultView, NavigationCaseReporter, ViewParamsReporter {
	...
	// almost ISO. Full ISO isn't available until Java 7. this uses -0400 where
	// ISO uses -04:00
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewParams, ComponentChecker checker) {
		...
		UIOutput.make(tofill, "datepicker")
				.decorate(new UIFreeAttributeDecorator("src", (majorVersion >= 10 ? "/library" : "/lessonbuilder-tool")
						+ "/js/lang-datepicker/lang-datepicker.js" + PortalUtils.getCDNQuery()));

		UIOutput.make(tofill, "portletBody")
				.decorate(new UIFreeAttributeDecorator("sakaimajor", Integer.toString(majorVersion)))
		...
		boolean canSeeAll = simplePageBean.canSeeAll(); // always on if
														// caneditpage
		...
		// can only do the fixups if you can write. We could hack permissions,
		// but
		...
		// the reason for a seaprate release date test is so we can show the
		// date.
		// there are currently some issues. If the page is not released and the
		// user doesn't have
		// access because of groups, this will show the not released data.
		// That's misleading because
		// when the release date comes the user still won't be able to see it.
		// Not sure if it's worth
		// creating a separate function that just checks the groups. It's easy
		// to test hidden, so I do that. The idea is that
		...
		if (!canSeeAll && currentPage.getReleaseDate() != null && currentPage.getReleaseDate().after(new Date())
			...
			String releaseMessage = messageLocator.getMessage("simplepage.not_yet_available_releasedate").replace("{}",
					releaseDate);
		...
		// the only thing not already tested (or tested in release check below)
		// in isItemVisible is groups. In theory
		...
		if (canEditPage) {
			...
			if (pageItem.getPageId() == 0 && !simplePageBean.isStudentPage(currentPage)) { // top
																							// level
																							// page
				// need dropdown
				...
				createToolBarLink(PermissionsHelperProducer.VIEW_ID, tofill, "permissions", "simplepage.permissions",
						currentPage, "simplepage.permissions.tooltip");
			...
			// Checks to see that user can edit and that this is either a top
			// level page,
			...
			if (simplePageBean.getEditPrivs() == 0 && (pageItem.getPageId() == 0)) {
				...
				// getEditPrivs < 2 if we want to let the student delete.
				// Currently we don't. There can be comments
			...
			UIOutput.make(tofill, "siteid", siteId);
			...
			// if this isn't a top level page, this will check that the page
			// above is
			// accessible. That matters because we check visible, available and
			// release
		...
		// this code is now for 11 only. helpurl is used in 9 and 10 to indicate
		// neo portal
		// at this point only two code paths are intended to work. inline and
		// iframe.
		// inline pushes stuff into the morpheus-generated header. iframe uses
		// an extra line
		// the previous mode required us to try to duplicate the header
		// generated by morpheus
		// this was too error-prone.
		String helpurl = null; /*
								 * (String)toolSession.getAttribute(
								 * "sakai-portal:help-action");
								 */
		String reseturl = null; /*
								 * (String)toolSession.getAttribute(
								 * "sakai-portal:reset-action");
								 */
		...
		if (helpurl != null) {
			UILink.make(tofill, (pageItem.getPageId() == 0 ? "helpbutton" : "helpbutton2"), helpurl)
					...
					.decorate(
							new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.help-button")));
			if (!inline)
			...
			UILink.make(tofill, "directurl")
					...
					.decorate(
							new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.direct-link")));
			// if (inline) {
			...
			// } else
			UIOutput.make(tofill, "directimage")
					.decorate(new UIFreeAttributeDecorator("alt", messageLocator.getMessage("simplepage.direct-link")));
		}
		...
		if (simplePageBean.isStudentPage(currentPage) && simplePageBean.getEditPrivs() == 0) {
			...
			if (student != null && pageItem.getGradebookId() != null) {
				...
				UIOutput.make(tofill, "topmaxpoints",
						String.valueOf((pageItem.getGradebookPoints() != null ? pageItem.getGradebookPoints() : "")));
				if (ownerName != null)
		...
		// Is anything visible?
		// Note that we don't need to check whether any item is available, since
		// the first visible
		...
		if (itemList.size() > 0) {
			...
			// Tho.Add.Start
			int itemVideoIndex = 0;
			// Tho.Add.End

			for (SimplePageItem i : itemList) {

				// break is not a normal item. handle it first
				// this will work whether first item is break or not. Might be a
				// section
				...
				if (first || i.getType() == SimplePageItem.BREAK) {
					...
					if (first || "section".equals(i.getFormat())) {
						...
						boolean collapsible = i.getAttribute("collapsible") != null
								&& (!"0".equals(i.getAttribute("collapsible")));
						boolean defaultClosed = i.getAttribute("defaultClosed") != null
								&& (!"0".equals(i.getAttribute("defaultClosed")));
						UIOutput sectionHeader = UIOutput.make(sectionWrapper, "sectionHeader");

						// only do this is there's an actual section break.
						// Implicit ones don't have an item to hold the title
						String headerText = "";
						...
						if (collapsible) {
							...
							sectionHeader.decorate(
									new UIFreeAttributeDecorator("aria-controls", sectionContainer.getFullID()));
							sectionHeader.decorate(
									new UIFreeAttributeDecorator("aria-expanded", (defaultClosed ? "false" : "true")));
					...
					if (canEditPage) {
						...
						if (i.getType() == SimplePageItem.BREAK)
							...
							// page doesn't start with a break. have to use
							// pageid
				...
				boolean listItem = !(i.getType() == SimplePageItem.TEXT || i.getType() == SimplePageItem.MULTIMEDIA
						...
						|| i.getType() == SimplePageItem.RESOURCE_FOLDER || i.getType() == SimplePageItem.CHECKLIST
						|| i.getType() == SimplePageItem.FORUM_SUMMARY || i.getType() == SimplePageItem.BREAK
				...
				case SimplePageItem.RESOURCE_FOLDER:
					itemClassName += "resourceFolderType";
					break;
				case SimplePageItem.PEEREVAL:
					itemClassName += "peereval";
					break;
				case SimplePageItem.TWITTER:
					itemClassName += "twitter";
					break;
				case SimplePageItem.FORUM_SUMMARY:
					itemClassName += "forumSummary";
					break;
				case SimplePageItem.ANNOUNCEMENTS:
					itemClassName += "announcementsType";
					break;
				case SimplePageItem.CALENDAR:
					itemClassName += "calendar";
					break;
				case SimplePageItem.CHECKLIST:
					itemClassName += "checklistType";
					break;
				// Hoctdy - Add Scorm Link - Start
				case SimplePageItem.SCORM:
					itemClassName += "scormType";
					break;
				// Hoctdy - Add Scorm Link - End
				}

				// inline LTI. Our code calls all BLTI items listItem, but the
				// inline version really isn't
				...
				if (listItem) { // Not an HTML Text, Element or Multimedia
					...
					// BLTI seems to require explicit specificaiton for column
					// width. Otherwise
					// we get 300 px wide. Don't know why. Doesn't happen to
					// other iframes
					if (isInline)
					...
					if (!isInline && !navButton && !"button".equals(i.getFormat())) {
						...
						case SimplePageItem.RESOURCE:
							...
							if (src == null) {
								src = "fa-file-o";
								// String image =
								// ContentTypeImageService.getContentTypeImage(mimeType);
								// if (image != null)
					...
					if (canEditPage) {
						UIOutput.make(tableRow, "edit-td").decorate(new UIFreeAttributeDecorator("title",
								messageLocator.getMessage("simplepage.edit-title.generic").replace("{}", i.getName())));
						UILink.make(tableRow, "edit-link", (String) null, "").decorate(new UIFreeAttributeDecorator(
								"title",
								messageLocator.getMessage("simplepage.edit-title.generic").replace("{}", i.getName())));
						...
						if (i.getType() == SimplePageItem.ASSIGNMENT) {
							...
							if (quiz != null) {
								...
								notPublished = quizEntity.notPublished(i.getSakaiId());
						} else if (i.getType() == SimplePageItem.SCORM) { // Hoctdy
																			// -
																			// Add
																			// Scorm
																			// Link
																			// -
																			// Start
							UIOutput.make(tableRow, "type", "scorm"); // Not
																		// used
																		// by
							// assignments,
							// so it is
							...
							if (quiz != null) {
								...
								notPublished = scormEntity.notPublished(i.getSakaiId());
							} // Hoctdy - Add Scorm Link - END
					...
					// This code should be read together with the code in
					// SimplePageBean
					// that sets up this data, method addMultimedia. Most
					// display is set
					// up here, but note that show-page.js invokes the jquery
					// oembed on all
					// <A> items with class="oembed".

					// historically this code was to display files ,and urls
					// leading to things
					// like MP4. as backup if we couldn't figure out what to do
					// we'd put something
					// in an iframe. The one exception is youtube, which we
					// supposed explicitly.
					// However we now support several ways to embed content. We
					// use the
					// multimediaDisplayType code to indicate which. The codes
					// are
					// 1 -- embed code, 2 -- av type, 3 -- oembed, 4 -- iframe
					// 2 is the original code: MP4, image, and as a special case
					// youtube urls
					// since we have old entries with no type code, and that
					// behave the same as
					// 2, we start by converting 2 to null.
					// then the logic is
					// if type == null & youtube, do youtube
					// if type == null & image, do iamge
					// if type == null & not HTML do MP4 or other player for
					// file
					// final fallthrough to handel the new types, with IFRAME if
					// all else fails
					// the old code creates ojbects in ContentHosting for both
					// files and URLs.
					// The new code saves the embed code or URL itself as an
					// atteibute of the item
					// If I were doing it again, I wouldn't create the
					// ContebtHosting item
					// Note that IFRAME is only used for something where the far
					// end claims the MIME
					// type is HTML. For weird stuff like MS Word files I use
					// the file display code, which
					...
					// String extension =
					// Validator.getFileExtension(i.getSakaiId());
					...
					if (mmDisplayType == null && simplePageBean.isImageType(i)) {
						// a wide default for images would produce completely
						// wrong effect
					...
					if (mmDisplayType == null && simplePageBean.isImageType(i)) {
						...
						if (canSeeAll || available) {
							if (!available)
								UIOutput.make(tableRow, "notAvailableText",
										messageLocator.getMessage("simplepage.fake-item-unavailable"));
							...
							item = UIOutput.make(tableRow, "image")
									.decorate(new UIFreeAttributeDecorator("src",
											i.getItemURL(siteId, currentPage.getOwner())))
						...
						if (canEditPage) {
							...
							UIOutput.make(tableRow, "imageWidth", widthSt); // original
																			// value
																			// from
																			// database
						...
						if (canSeeAll || simplePageBean.isItemAvailable(i)) {
							...
							if (lengthOk(height)) {
								...
								// Youtube seems to use aspect ratio of 16*9
								// from 2015 on
						...
						if (canEditPage) {
							...
							UIOutput.make(tableRow, "youtube-td").decorate(new UIFreeAttributeDecorator("title",
						...
						// Youtube, or HTML
						// This could be audio, video, flash, or something
						// random like MS word.
						...
						if (canSeeAll || available) {
							...
							String movieUrl = i.getItemURL(siteId, currentPage.getOwner());
							// movieUrl = "https://heidelberg.rutgers.edu" +
							// movieUrl;
							// Safari doens't always pass cookies to plugins, so
							// we have to pass the arg
							...
							// isMp4 means we try the flash player (if not
							// HTML5)
							// we also try the flash player for FLV but for mp4
							// we do an
							// additional backup if flash fails, but that
							// doesn't make sense for FLV
							boolean isMp4 = Arrays.binarySearch(mp4Types, mimeType) >= 0;
							boolean isHtml5 = Arrays.binarySearch(html5Types, mimeType) >= 0;

							// wrap whatever stuff we decide to put out in HTML5
							// if appropriate
							// javascript is used to do the wrapping, because
							// RSF can't really handle this
							if (isHtml5) {
								// flag for javascript
								...
								// HTML5 spec says % isn't legal in width, so
								// have to use style
								String style = null;
								if (lengthOk(height))
								...
								// Tho.Add.Start.Script to tracking %
								// ontimeupdate="trackProgress(this,
								// movieUrl,id)"
								String ontimeupdate = "trackProgress(this,'" + siteId + "','" + movieUrl + "',"
										+ i.getId() + ")";
								h5video.decorate(new UIFreeAttributeDecorator("ontimeupdate", ontimeupdate));

								// Support to set the position at the last play
								// position when init the video
								h5video.decorate(new UIFreeAttributeDecorator("id", "video" + itemVideoIndex));

								// Support display the loading icon
								String onloadstart = "loadStart(' + i.getId() + ')";
								h5video.decorate(new UIFreeAttributeDecorator("onloadstart", onloadstart));

								String oncanplay = "canPlay(' + i.getId() + ')";
								h5video.decorate(new UIFreeAttributeDecorator("oncanplay", oncanplay));

								// Add hidden input to store the start position
								// of video
								Double percent;

								// Get the learned progress of user.
								String userId = UserDirectoryService.getCurrentUser().getId();
								percent = simplePageToolDao.getLearnedProgress(i.getId(), siteId, userId);
								if (percent == null) {
									percent = 0.0;
								}

								UIComponent videoStartPos = UIOutput.make(tableRow, "video_start_pos");
								videoStartPos.decorate(new UIFreeAttributeDecorator("id", "video_start_pos"))
										.decorate(new UIFreeAttributeDecorator("value", String.valueOf(percent)));
								// Add id attribute
								itemVideoIndex++;
								// Tho.Add.End
								...
								if (!isAudio && caption != null && caption.length() > 0) {
									...
									// sessionParameter should always be
									// non-null
									// because this overrides all other checks
									// in /access/lessonbuilder,
									// we haven't adjusted it to handle these
									// files otherwise
									if (sessionParameter != null)
							...
							// FLV is special. There's no player for flash video
							// in
							// the browser
							...
							// for IE, if we're not supplying a player it's
							// safest
							// to use embed
							// otherwise Quicktime won't work. Oddly, with IE 9
							// only
							// it works if you set CLASSID to the MIME type,
							// but that's so unexpected that I hate to rely on
							// it.
							// EMBED is in HTML 5, so I think we're OK
							// using it permanently for IE.
							// I prefer OBJECT where possible because of the
							// nesting
							// ability.
							...
							if (canEditPage) {
								// item2.decorate(new
								// UIFreeAttributeDecorator("style", "border:
								// 1px solid black"));
							}

							// some object types seem to need a specification,
							// so supply our default if necessary
							if (lengthOk(height) && lengthOk(width)) {
								...
								// this is mostly because the default is 640
								// with no height specified
								// we've validated width, so no errors in
								// conversion should occur
								Double h = new Double(width.getOld()) * 0.75;
								if (oMimeType.startsWith("audio/"))
									h = 100.0;
								item2.decorate(new UIFreeAttributeDecorator("height", Double.toString(h)))
										.decorate(new UIFreeAttributeDecorator("width", width.getOld()));
							...
							if (!useEmbed) {
								if (useFlvPlayer) {
									UIOutput.make(tableRow, "flashvars")
											.decorate(new UIFreeAttributeDecorator("value", "src=" + URLEncoder
													.encode(myUrl() + i.getItemURL(siteId, currentPage.getOwner()))));
									...
									// need ultimate performance for embedded
									// video. I'm setting it only for
							...
							if (isMp4) {
								...
								// some object types seem to need a
								// specification, so give a default if needed
								if (lengthOk(height) && lengthOk(width)) {
									item2.decorate(new UIFreeAttributeDecorator("height", height.getOld()))
											.decorate(new UIFreeAttributeDecorator("width", width.getOld()));
								} else if (definiteLength(width)) {
								...
								if (!useEmbed) {
									UIOutput.make(tableRow, "mp4-inject").decorate(new UIFreeAttributeDecorator("value",
											i.getItemURL(siteId, currentPage.getOwner())));

									if (showDownloads) {
										UIOutput.make(tableRow, "mp4-noplugin-p",
												messageLocator.getMessage("simplepage.noplugin"));
										UILink.make(tableRow, "mp4-noplugin", i.getName(),
												i.getItemURL(siteId, currentPage.getOwner()));
									}
								}
							}
						...
						if (canEditPage) {
							...
							UILink.make(tableRow, "edit-movie", (String) null, "")
									.decorate(new UIFreeAttributeDecorator("title",
											messageLocator.getMessage("simplepage.edit-title.url").replace("{}",
													abbrevUrl(i.getURL()))));
							// Add link to export learning progress
							String exportUrl = String.format(
									"/lessonbuilder-tool/ajax?op=exportprogress&siteid=%s&itemid=%d", siteId,
									i.getId());
							String downloadTitle = messageLocator.getMessage("simplepage.export-title.generic")
									.replace("{}", i.getName());
							UILink.make(tableRow, "export-progress-link")
									.decorate(new UIFreeAttributeDecorator("href", exportUrl))
									.decorate(new UIFreeAttributeDecorator("title", downloadTitle));
						}
						// Tho.Add.Start.Display learned progress
						// Display for both Student and Instructor (Has edit
						// permission)
						// else {
						Double percent;
						...
						UIOutput.make(tableRow, "progress-td")
								.decorate(new UIFreeAttributeDecorator("title", "Learning progress"));

						if (percent == null) {
							percent = 0.0;
						}

						String iconLabel = (percent > 80) ? "progressIdCompleted"
								: ((percent > 0.0) ? "progressIdOnGoing" : "progressIdNone");
						// UIOutput.make(tableRow, iconLabel,
						// String.format("%d%%",Math.round(percent)));
						// UIOutput.make(tableRow, iconLabel).decorate(new
						// UIFreeAttributeDecorator("id", i.getId() + "_" +
						// "progressId"));
						UIOutput.make(tableRow, iconLabel, String.format(" %d%%", Math.round(percent)))
								.decorate(new UIFreeAttributeDecorator("id", i.getId() + "_" + "progressId"));

						// Show Export progress bar
						// UIOutput.make(tableRow,
						// "export-progress-td").decorate(new
						// UIFreeAttributeDecorator("title","Export learning
						// progress"));

						// String exportUrl =
						// String.format("/lessonbuilder-tool/ajax?op=exportprogress&siteid=%s&itemid=%d",
						// siteId, i.getId());

						// UILink.make(tableRow, "export-progress-link",
						// messageLocator.getMessage("simplepage.export_button"),
						// null).decorate(new UIFreeAttributeDecorator("href",
						// exportUrl));
						// UILink.make(tableRow,
						// "export-progress-link").decorate(new
						// UIFreeAttributeDecorator("href", exportUrl));
						// }
						// Tho.Add.End

					} else {
						// this is fallthrough for html or an explicit mm
						// display type (i.e. embed code)
						// odd types such as MS word will be handled by the AV
						// code, and presented as <OBJECT>
						...
						if (canSeeAll || available) {
							...
							String itemUrl = i.getItemURL(siteId, currentPage.getOwner());
							if ("1".equals(mmDisplayType)) {
								...
								// if (style != null)
								// item.decorate(new
								// UIFreeAttributeDecorator("style", style));
								...
								// if user specifies auto, use Javascript to
								// resize the
								// iframe when the
								// content changes. This only works for URLs
								// with the
								// same origin, i.e.
					...
					if (!isAvailable && !canSeeAll) {
						...
						if (canEditPage) {
							...
							if (i.getPageId() > 0 && i.getGradebookId() != null && !cameFromGradingPane) {
								...
								UIInternalLink.make(tableRow, "gradingPaneLink", gp)
										.decorate(new UIFreeAttributeDecorator("title",
												messageLocator.getMessage("simplepage.show-grading-pane-comments")));
							}

							UIOutput.make(tableRow, "comments-td").decorate(new UIFreeAttributeDecorator("title",
						...
						if (!noEditor) {
							fckInput.decorate(new UIStyleDecorator("using-editor")); // javascript
																						// needs
																						// to
																						// know
					...
					if (isOpen) {
						...
						boolean evalIndividual = (i.isGroupOwned()
								&& "true".equals(i.getAttribute("group-eval-individual")));

						// If groupMembers is empty this should be true
						// (individual) even if this is set to being in a group
						...
						// group owned and eval individual and we're in the
						// group
						...
						if (!(evalIndividual && !groupMembers.contains(currentUser))) {
							...
							UIOutput.make(tableRow, "peer-eval-title-student",
									String.valueOf(i.getAttribute("rubricTitle")));
							UIForm peerForm = UIForm.make(tableRow, "peer-review-form");
							UIInput.make(peerForm, "peer-eval-itemid", "#{simplePageBean.itemId}",
									String.valueOf(i.getId()));

							// originally evalTargets was a list if ID's.
							// but we need to sort by name, so unless we want to
							// keep repeatedly
							...
							if (evalIndividual) {
								...
								// no need to sort for other alternative, when
								// there's only one
								...
								// individual handled above. So if group we're
								// evaluating
							...
							// for old format entries always need page owner or
							// evaluee
							...
							for (Target target : evalTargets) {
								...
								// current data to show to target, all
								// evaluations of target
								// But first see if we should show current data.
								// Only show
								...
								if ((i.isGroupOwned() && !evalIndividual && groupMembers.contains(currentUser))
									...
									List<SimplePagePeerEvalResult> evaluations = simplePageToolDao
											.findPeerEvalResultByOwner(pageId.longValue(), target.id, groupId);

									if (evaluations != null) {
										for (SimplePagePeerEvalResult eval : evaluations) {
											// for individual eval only show
								...
								// now get current data to initiaize form.
								// That's just
								// the submission by current user.
								List<SimplePagePeerEvalResult> evaluations = simplePageToolDao
										.findPeerEvalResult(pageId, currentUser, target.id, groupId);
								...
								// keep this is sync with canSubmit in
								// SimplePageBean.savePeerEvalResult
								boolean canSubmit = (!i.isGroupOwned() && (!owner.equals(currentUser) || gradingSelf)
										|| i.isGroupOwned() && !evalIndividual
												&& (!groupMembers.contains(currentUser) || peerEvalAllowSelfGrade)
										|| evalIndividual && groupMembers.contains(currentUser)
												&& (peerEvalAllowSelfGrade || !target.id.equals(currentUser)));

								makePeerRubric(entry, i, makeStudentRubric, selectedCells, dataMap, canSubmit);

							}

							// can submit
							// individual and (not that individual or
							// gradingeself)
							...
							if (!i.isGroupOwned() && (!owner.equals(currentUser) || gradingSelf)
								...
								if (isPastDue) {
									...
									UICommand.make(peerForm, "save-peereval-link",
											messageLocator.getMessage("simplepage.submit"),
											"#{simplePageBean.savePeerEvalResult}");
									UIOutput.make(peerForm, "save-peereval-text",
											messageLocator.getMessage("simplepage.save"));
									UIOutput.make(peerForm, "cancel-peereval-link");
									UIOutput.make(peerForm, "cancel-peereval-text",
								...
								// in theory the only case where we show the
								// form and can't grade
						...
						if (canEditPage)
							UIOutput.make(tableRow, "peerReviewRubricStudent-edit");// lines
																					// up
																					// rubric
																					// with
																					// edit
					...
					if (!isAvailable && !canSeeAll)
						...
						// notSubmitted will be list of students or groups that
						// didn't submit. Start with those who
						...
						for (SimpleStudentPage page : studentPages) {
							...
							if (i.getShowComments() != null && i.getShowComments()) {
								...
								if (page.getLastCommentChange() != null && (entry == null
									...
									UIOutput.make(row, "newCommentsImg").decorate(new UIFreeAttributeDecorator("title",
											messageLocator.getMessage("simplepage.new-student-comments")));
								} else
							...
							if (entry == null) {
								UIOutput.make(row, "newPageImg").decorate(new UIFreeAttributeDecorator("title",
										messageLocator.getMessage("simplepage.new-student-content-page")));
							} else
						...
						if (simplePageBean.getEditPrivs() == 0) {
							if (notSubmitted.size() > 0) {
								UIBranchContainer row = UIBranchContainer.make(tableRow, "studentRow:");
								UIOutput.make(row, "missingStudentTitle",
										messageLocator.getMessage("simplepage.missing-students"));
							}
							...
							for (String owner : notSubmitted) {
								...
								if (i.isGroupOwned()) {
									try {
										...
										// the only way I can make this happen
										// is to add a group
										// to the item and then delete the
										// group. If we can't find the
									...
									try {
										...
										// can't find user, just show userid.
										// Not very useful, but at least shows
										// what happened
										sownerName = owner;
									}
								}
							...
							if (notSubmitted.size() > 0 && i.getGradebookId() != null) {
								...
								UIInput.make(zeroForm, "student-zero-item", "#{simplePageBean.itemId}",
										String.valueOf(i.getId()));
								UICommand.make(zeroForm, "student-zero",
						...
						if (!hasOwnPage && simplePageBean.myStudentPageGroupsOk(i)) {
							...
							UICommand.make(studentForm, "add-content", messageLocator.getMessage("simplepage.add-page"),
									"#{simplePageBean.createStudentPage}");
							;
						}

						String itemGroupString = null;
						// do before canEditAll because we need itemGroupString
						// in it
						if (canSeeAll) {
							itemGroupString = simplePageBean.getItemGroupString(i, null, true);
							if (itemGroupString != null) {
						...
						if (canEditPage) {
							...
							UIOutput.make(tableRow, "student-td").decorate(new UIFreeAttributeDecorator("title",
							...
							if (peerEvalDate != null && peerDueDate != null) {
								...
								// log.info("Setting date to " + dateStr + " and
								// time to " + timeStr);
							...
							UIOutput.make(tableRow, "student-group-owned-eval-individual",
									(i.getAttribute("group-eval-individual")));
							UIOutput.make(tableRow, "student-group-owned-see-only-own",
									(i.getAttribute("see-only-own")));
						}
					}
					...
					boolean available = simplePageBean.isItemAvailable(i);
					// note that there's currently no UI for setting
					// availability check
					if (canSeeAll || available) {
						if (!available)
							UIOutput.make(tableRow, "notAvailableText",
						...
						// setting announcements url to get all announcements
						// for the site
						UIOutput.make(tableRow, "announcements-site-url",
								myUrl() + "/direct/announcement/site/" + simplePageBean.getCurrentSiteId());
						// setting this variable to redirect user to the
						// particular announcement
						UIOutput.make(tableRow, "announcements-view-url", myUrl() + "/portal/directtool/"
								+ simplePageBean.getCurrentTool(simplePageBean.ANNOUNCEMENTS_TOOL_ID)
								+ "?itemReference=/announcement/msg/" + simplePageBean.getCurrentSiteId() + "/main/");
						// get numberOfAnnouncements for the widget
						String numberOfAnnouncements = i.getAttribute("numberOfAnnouncements") != null
								? i.getAttribute("numberOfAnnouncements") : "";
					...
					if (canSeeAll || available) {
						...
						// setting forums-messages url to get all recent
						// messages for the site
						UIOutput.make(tableRow, "forum-summary-site-url",
								myUrl() + "/direct/forums/messages/" + simplePageBean.getCurrentSiteId());
						// setting the url such that request goes to
						// ShowItemProducer to display forum conversations
						// inside Lessons tool
						UIOutput.make(tableRow, "forum-summary-view-url",
								myUrl() + "/portal/site/" + simplePageBean.getCurrentSiteId() + "/tool/"
										+ placement.getId() + "/" + ShowItemProducer.VIEW_ID + "?itemId=" + i.getId()
										+ "&sendingPage=" + currentPage.getPageId());
						// get numberOfConversations for the widget
						String numberOfConversations = i.getAttribute("numberOfConversations") != null
								? i.getAttribute("numberOfConversations") : "";
					...
					if (canSeeAll || simplePageBean.isItemAvailable(i)) {
						...
						String html = "<p><b>" + folderName
								+ "</b></p><div data-copyright=\"true\" class=\"no-highlight\" data-description=\"true\" data-directory='"
								+ dataDirectory + "' data-files=\"true\" data-folder-listing=\"true\"></div>";
						UIVerbatim.make(tableRow, "content", html);
					} else {
					...
					if (canSeeAll) {
						...
						if (canEditPage) {
							...
							String customCssClass = i.getAttribute("customCssClass") != null
									? i.getAttribute("customCssClass") : "";
					...
					if (canSeeAll || simplePageBean.isItemAvailable(i)) {
						...
						UIOutput.make(tableRow, "site-events-url",
								myUrl() + "/direct/calendar/site/" + simplePageBean.getCurrentSiteId());
						// set calendar tool url for more info of calendar event
						UIOutput.make(tableRow, "event-tool-url", myUrl() + "/portal/directtool/"
								+ simplePageBean.getCurrentTool(simplePageBean.CALENDAR_TOOL_ID) + "?eventReference=");
					} else {
						UIComponent unavailableText = UIOutput.make(tableRow, "content",
								messageLocator.getMessage("simplepage.textItemUnavailable"));
						unavailableText.decorate(new UIFreeAttributeDecorator("class", "disabled-text-item"));
					...
					if ("multipleChoice".equals(i.getAttribute("questionType"))) {
						...
						UISelect multipleChoiceSelect = UISelect.make(questionForm, "multipleChoiceSelect:", options,
								"#{simplePageBean.questionResponse}", initValue);
						...
						for (int j = 0; j < answers.size(); j++) {
							...
							UIOutput.make(answerContainer, "multipleChoiceAnswerText",
									Integer.toString(j + 1) + " : " + answers.get(j).getText())
					...
					if ("multipleChoice".equals(i.getAttribute("questionType"))
						...
						// in theory we don't need the first loop, as there
						// should be a total
						// entry for all possible answers. But in case things
						// are out of sync ...
						...
						for (int j = 0; j < answers.size(); j++) {
							...
							UIOutput.make(pollContainer, "questionPollLegend",
									Integer.toString(j + 1) + ":" + answers.get(j).getText());
					...
					if (canEditPage) {
						UIOutput.make(tableRow, "question-td").decorate(new UIFreeAttributeDecorator("title",
								messageLocator.getMessage("simplepage.edit-title.question")));

						// always show grading panel. Currently this is the only
						// way to get feedback
						if (!cameFromGradingPane) {
						...
						UILink.make(tableRow, "edit-question", (String) null, "").decorate(new UIFreeAttributeDecorator(
								"title", messageLocator.getMessage("simplepage.edit-title.question")));
					...
					for (SimpleChecklistItem checklistItem : checklistItems) {
						...
						if (checklistItem.getLink() > 0L) {
							...
							if (linkedItem != null) {
								...
								ChecklistItemStatus status = simplePageToolDao.findChecklistItemStatus(i.getId(),
										checklistItem.getId(), simplePageBean.getCurrentUserId());
								if (Status.COMPLETED.equals(linkedItemStatus)) {
									if (status != null) {
										...
										status = new ChecklistItemStatusImpl(i.getId(), checklistItem.getId(),
												simplePageBean.getCurrentUserId());
										status.setDone(true);
										simplePageToolDao.saveChecklistItemStatus(status);
									}
						...
						boolean isDone = simplePageToolDao.isChecklistItemChecked(i.getId(), checklistItem.getId(),
								simplePageBean.getCurrentUserId());
					...
					if (!checklistItems.isEmpty()) {
						UISelect select = UISelect.makeMultiple(checklistForm, "checklist-span",
								values.toArray(new String[1]), "#{simplePageBean.selectedChecklistItems}",
								initValues.toArray(new String[1]));

						int index = 0;
						for (SimpleChecklistItem checklistItem : checklistItems) {
							UIBranchContainer row = UIBranchContainer.make(checklistForm, "select-checklistitem-list:");
							UIComponent input = UISelectChoice
									.make(row, "select-checklistitem", select.getFullID(), index)
									.decorate(new UIStyleDecorator("checklist-checkbox"));
							...
							if (checklistItem.getLink() > 0L) {
								...
								if (linkedItem != null) {
									...
									String tooltipContent = messageLocator.getMessage(toolTipMessage).replace("{}",
											SimplePageItemUtilities.getDisplayName(linkedItem));

									if (!simplePageBean.isItemVisible(linkedItem)) {
										row.decorate(new UIStyleDecorator("checklist-blur"));
										tooltipContent = messageLocator
												.getMessage("simplepage.checklist.external.link.details.notvisible");
										checklistItemName = messageLocator
												.getMessage("simplepage.checklist.external.link.hidden");
									}
							...
							UIOutput.make(row, "select-checklistitem-name", checklistItemName)
									.decorate(new UIStyleDecorator("checklist-checkbox-label"));
							index++;
						}
					}
					...
					if (canEditPage) {
						...
						UIOutput.make(tableRow, "editchecklist-td").decorate(new UIFreeAttributeDecorator("title",
								messageLocator.getMessage("simplepage.edit-checklist").replace("{}", i.getName())));

						GeneralViewParameters eParams = new GeneralViewParameters();
						eParams.setSendingPage(currentPage.getPageId());
						eParams.setItemId(i.getId());
						eParams.viewID = ChecklistProducer.VIEW_ID;
						UIInternalLink.make(tableRow, "edit-checklist", (String) null, eParams)
								.decorate(new UIFreeAttributeDecorator("title", messageLocator
										.getMessage("simplepage.edit-checklist").replace("{}", i.getName())));
						...
						UIInternalLink.make(tableRow, "edit-checklistProgress", (String) null, gvp)
								.decorate(new UIFreeAttributeDecorator("title", messageLocator
										.getMessage("simplepage.view-checklist").replace("{}", i.getName())));
					}
					...
					if (canSeeAll || simplePageBean.isItemAvailable(i)) {
						// Getting variables from attributes of this twitter
						// page item
						String tweetLimit = i.getAttribute("numberOfTweets") != null ? i.getAttribute("numberOfTweets")
								: "5";
						...
						String html = "<div align=\"left\" style='" + divHeight
								+ "' class=\"twitter-div\"><a class=\"twitter-timeline\" href= '" + href
								+ "' data-widget-id='" + ServerConfigurationService.getString(TWITTER_WIDGET_ID)
								+ "'  data-tweet-limit='" + tweetLimit + "' data-dnt=\"true\" data-screen-name='"
								+ username + "'>Tweets by @'" + username + "'</a></div>";
						UIVerbatim.make(tableRow, "content", html);
			...
			if (canSeeAll) {
				...
				if (currentPage.isHidden()) {
					UIOutput.make(tofill, "hiddenAlert").decorate(
							new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.pagehidden")));
					...
					String releaseDate = df.format(currentPage.getReleaseDate());
					UIOutput.make(tofill, "hiddenAlert").decorate(
							new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.notreleased")));
		...
		// Add pageids to the page so the portal lessons subnav menu can update
		// its state
		List<SimplePageBean.PathEntry> path = simplePageBean.getHierarchy();
		if (path.size() > 2) {
			...
			UIOutput.make(tofill, "lessonsSubnavPageId").decorate(
					new UIFreeAttributeDecorator("value", String.valueOf(simplePageBean.getCurrentPage().getPageId())));
		}
		...
		UIOutput.make(tofill, "lessonsCurrentPageId").decorate(
				new UIFreeAttributeDecorator("value", String.valueOf(simplePageBean.getCurrentPage().getPageId())));
	}
	...
	private boolean makeLink(UIContainer container, String ID, SimplePageItem i, boolean canEditPage,
			SimplePage currentPage, boolean notDone, Status status) {
		return makeLink(container, ID, i, simplePageBean, simplePageToolDao, messageLocator, canEditPage, currentPage,
				notDone, status);
	}
	 ...
	 */
	protected static boolean makeLink(UIContainer container, String ID, SimplePageItem i, SimplePageBean simplePageBean,
		...
		if (i.getSakaiId().equals(SimplePageItem.DUMMY)) {
			...
			if (usable) {
				if (i.getType() == SimplePageItem.RESOURCE && i.isSameWindow()) {
					...
					// run this through /access/lessonbuilder so we can track it
					// even if the user uses the context menu
					// We could do this only for the notDone case, but I think
					// it could cause trouble for power users
					// if the url isn't always consistent.
					if (i.getAttribute("multimediaUrl") != null) { // resource
																	// where
																	// we've
																	// stored
																	// the URL
			...
			if (p != null) {
				...
				if (canEditPage || (p.getOwner() != null && p.getOwner().equals(simplePageBean.getCurrentUserId()))) {
					...
					if (StringUtils.isNotBlank(owner)) {
						UIOutput.make(container, "owner-text",
								"(" + messageLocator.getMessage("simplepage.owner.settings") + ": " + owner + ")");
					}
				}
			} else {
			...
			// assignments won't let us get the entity if we're not in the
			// group, so set up permissions before other tests
		...
		} else if (i.getType() == SimplePageItem.SCORM) { // Hoctdy - Add Scorm
															// Link - Start
			// assignments won't let us get the entity if we're not in the
			// group, so set up permissions before other tests
			...
			if (available && lessonEntity != null && (canEditPage || !scormEntity.notPublished(i.getSakaiId()))) {
				// we've hacked Samigo to look at a special lesson builder
				// session
				// attribute. otherwise at the end of the test, Samigo replaces
				// the
				...
				GeneralViewParameters view = new GeneralViewParameters(ShowItemProducer.VIEW_ID);
				view.setSendingPage(currentPage.getPageId());
				view.setClearAttr("LESSONBUILDER_RETURNURL_SCORM");
				view.setItemId(i.getId());
				UILink link = UIInternalLink.make(container, "link", lessonEntity.getUrl());
				link.decorate(new UIFreeAttributeDecorator("lessonbuilderitem", itemString));
				link.decorate(new UIFreeAttributeDecorator("target", "_blank"));
			} else {
				if (i.isPrerequisite()) {
			...
			if ("inline".equals(i.getFormat())) {
				...
				if (usable && lessonEntity != null) {
					// I'm fairly sure checkitempermissions doesn't do anything
					// useful for LTI,
				...
				if (usable && lessonEntity != null) {
					...
					// ID, i.getName(), URL, i.getId(), notDone);
		...
		if (fake) {
			...
			link.decorate(new UIFreeAttributeDecorator("lessonbuilderitem", itemString));
			// fake and available occurs when prerequisites aren't the issue
			// (it's avaiable)
	...
	private void createTwitterDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "add-twitter-dialog")
				.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.twitter")));
		...
		UICommand.make(form, "twitter-add-item", messageLocator.getMessage("simplepage.save_message"),
				"#{simplePageBean.addTwitterTimeline}");
		UICommand.make(form, "twitter-cancel", messageLocator.getMessage("simplepage.cancel"), null);
		UICommand.make(form, "delete-twitter-item", messageLocator.getMessage("simplepage.delete"),
	...
	private void styleItem(UIContainer row, UIComponent container, UIComponent component, SimplePageItem i,
			String indent, String customClass) {
		// Indent level - default to 0 if not previously set
		String indentLevel = i.getAttribute(SimplePageItem.INDENT) == null ? "0"
				: i.getAttribute(SimplePageItem.INDENT);
	...
	// Create a latest forum conversations dialog where user can enter other
	// settings for the forum summary div
	private void createForumSummaryDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "add-forum-summary-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.forumSummaryLinkText")));
		UIForm form = UIForm.make(tofill, "add-forum-summary-form");
		makeCsrf(form, "csrf24");
		// check if site has forum tool added?if not then display info and
		// return
		...
		UIOutput.make(form, "forumNumberDropdownLabel",
				messageLocator.getMessage("simplepage.forum-number-dropdown-label"));
		UISelect.make(form, "forumNumberDropdown", options, labels, "#{simplePageBean.forumSummaryDropDown}", "5");
		UICommand.make(form, "forum-summary-add-item", messageLocator.getMessage("simplepage.save_message"),
				"#{simplePageBean.addForumSummary}");
		UIInput.make(form, "forum-summary-add-before", "#{simplePageBean.addBefore}");
		UICommand.make(form, "forum-summary-cancel", messageLocator.getMessage("simplepage.cancel"), null);
		UICommand.make(form, "delete-forum-summary-item", messageLocator.getMessage("simplepage.delete"),
				"#{simplePageBean.deleteItem}");
	}
	...
	/**
	 * Checks for the version of IE. Returns 0 if we're not running IE. But
	 * there's a problem. IE 11 doesn't have the MSIE tag. But it stiill needs
	 * to be treated as IE, because the OBJECT tag won't work with Quicktime
	...
	private void createToolBar(UIContainer tofill, SimplePage currentPage, boolean isStudent) {
		...
		// right side
		createToolBarLink(ReorderProducer.VIEW_ID, toolBar, "reorder", "simplepage.reorder", currentPage,
				"simplepage.reorder-tooltip");

		// add content menu
		createToolBarLink(EditPageProducer.VIEW_ID, tofill, "add-text1", null, currentPage, "simplepage.text.tooltip")
				.setItemId(null);
		createFilePickerToolBarLink(ResourcePickerProducer.VIEW_ID, tofill, "add-resource1", null, false, false,
				currentPage, "simplepage.resource.tooltip");
		createFilePickerToolBarLink(ResourcePickerProducer.VIEW_ID, tofill, "add-multimedia1", null, true, false,
		...
		createToolBarLink(EditPageProducer.VIEW_ID, tofill, "add-text", "simplepage.text", currentPage,
				"simplepage.text.tooltip").setItemId(null);
		createFilePickerToolBarLink(ResourcePickerProducer.VIEW_ID, tofill, "add-multimedia", "simplepage.multimedia",
				true, false, currentPage, "simplepage.multimedia.tooltip");
		createFilePickerToolBarLink(ResourcePickerProducer.VIEW_ID, tofill, "add-resource", "simplepage.resource",
				false, false, currentPage, "simplepage.resource.tooltip");

		boolean showAddResourceFolderLink = ServerConfigurationService
				.getBoolean("lessonbuilder.show.resource.folder.link", true);
		if (showAddResourceFolderLink) {
			createToolBarLink(FolderPickerProducer.VIEW_ID, tofill, "add-folder", "simplepage.folder", currentPage,
					"simplepage.addfolder.tooltip").setItemId(null);
		}
		...
		if (!studentPage) {
			...
			try {
				Method expandMethod = contentHostingInterface.getMethod("expandZippedResource",
						new Class[] { String.class });

				UIOutput.make(tofill, "addwebsite-li");
				createFilePickerToolBarLink(ResourcePickerProducer.VIEW_ID, tofill, "add-website", "simplepage.website",
						false, true, currentPage, "simplepage.website.tooltip");
			...
			announcementsLink
					.decorate(new UITooltipDecorator(messageLocator.getMessage("simplepage.announcements-descrip")));
			UIOutput.make(tofill, "assignment-li");
			createToolBarLink(AssignmentPickerProducer.VIEW_ID, tofill, "add-assignment",
					"simplepage.assignment-descrip", currentPage, "simplepage.assignment");

			boolean showEmbedCalendarLink = ServerConfigurationService.getBoolean("lessonbuilder.show.calendar.link",
					true);
			if (showEmbedCalendarLink) {
				UIOutput.make(tofill, "calendar-li");
				UIOutput.make(tofill, "calendar-link")
						.decorate(new UITooltipDecorator(messageLocator.getMessage("simplepage.calendar-descrip")));
			...
			createToolBarLink(QuizPickerProducer.VIEW_ID, tofill, "add-quiz", "simplepage.quiz-descrip", currentPage,
					"simplepage.quiz");

			// Hoctdy - add scrom link - start
			UIOutput.make(tofill, "scorm-content-li");
			createToolBarLink(ScormPickerProducer.VIEW_ID, tofill, "add-scorm-content", "simplepage.scorm.resource",
					currentPage, "simplepage.scorm.resource");
			// Hoctdy - add scrom link - end
			...
			forumSummaryLink
					.decorate(new UITooltipDecorator(messageLocator.getMessage("simplepage.forum-summary-descrip")));

			UIOutput.make(tofill, "forum-li");
			createToolBarLink(ForumPickerProducer.VIEW_ID, tofill, "add-forum", "simplepage.forum-descrip", currentPage,
					"simplepage.forum.tooltip");

			UIOutput.make(tofill, "checklist-li");
			createToolBarLink(ChecklistProducer.VIEW_ID, tofill, "add-checklist", "simplepage.checklist", currentPage,
			...
			UIOutput.make(tofill, "add-comments-link")
			...
			UIOutput.make(tofill, "add-student-link")
			...
			boolean showEmbedTwitterLink = ServerConfigurationService.getBoolean("lessonbuilder.show.twitter.link",
					false);
			...
			if (bltiEntity != null && ((BltiInterface) bltiEntity).servicePresent()) {
				...
				createToolBarLink(BltiPickerProducer.VIEW_ID, tofill, "add-blti", "simplepage.blti", currentPage,
						"simplepage.blti.tooltip");
			}

		}
	}

	private GeneralViewParameters createToolBarLink(String viewID, UIContainer tofill, String ID, String message,
	...
	private FilePickerViewParameters createFilePickerToolBarLink(String viewID, UIContainer tofill, String ID,
			String message, boolean resourceType, boolean website, SimplePage currentPage, String tooltip) {
	...
	private void createStandardToolBarLink(String viewID, UIContainer tofill, String ID, String message,
			SimpleViewParameters params, String tooltip) {
	...
	private void createSubpageDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "subpage-dialog")
				.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.subpage")));
		...
		if (!simplePageBean.isStudentPage(currentPage)) {
			UIInternalLink.make(form, "subpage-choose", messageLocator.getMessage("simplepage.choose_existing_page"),
					view);
		}
	...
	private void createEditItemDialog(UIContainer tofill, SimplePage currentPage, SimplePageItem pageItem) {
		...
		String returnFromEditString = messageLocator.getMessage("simplepage.return_from_edit").replace("{}",
				currentToolTitle);

		UIOutput.make(tofill, "edit-item-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.edititem_header")));
		...
		UIInternalLink.make(form, "change-assignment", messageLocator.getMessage("simplepage.change_assignment"),
				params);
		...
		// Hoctdy - Add Scorm Link - Edit Start
		params = new GeneralViewParameters();
		params.setSendingPage(currentPage.getPageId());
		...
		// Hoctdy - Add Scorm Link - Edit End
		...
		UIInternalLink.make(form, "change-resource", messageLocator.getMessage("simplepage.change_resource"),
				fileparams);
		...
		// Hoctdy - Add Scorm Link - Edit Start
		if (pageItem.getType() != SimplePageItem.SCORM) {
		...
		UISelect radios = UISelect.make(form, "format-select", new String[] { "window", "inline", "page" },
		...
		UISelect.make(form, "assignment-dropdown", SimplePageBean.GRADES, "#{simplePageBean.dropDown}",
				SimplePageBean.GRADES[0]);
	...
	public void createGroupList(UIContainer tofill, Collection<String> groupsSet, String prefix, String beanspec) {
		...
		UISelect select = UISelect.makeMultiple(tofill, prefix + "group-list-span", values.toArray(new String[1]),
				beanspec, initValues.toArray(new String[1]));
	...
	private void createAnnouncementsDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "add-announcements-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.announcementsLinkText")));
		UIForm form = UIForm.make(tofill, "add-announcements-form");
		makeCsrf(form, "csrf23");
		// check if site has announcements tool added?if not then display info
		// and return
		if (simplePageBean.getCurrentTool(simplePageBean.ANNOUNCEMENTS_TOOL_ID) == null) {
			UIOutput.make(tofill, "announcements-error-div");
			UIOutput.make(tofill, "announcements-error-span",
					messageLocator.getMessage("simplepage.no_announcements_tool"));
			UICommand.make(form, "announcements-cancel", messageLocator.getMessage("simplepage.cancel"), null);
			UICommand.make(form, "delete-announcements-item", messageLocator.getMessage("simplepage.delete"),
					"#{simplePageBean.deleteItem}");
			return;
		}
		...
		String[] options = { "5", "10", "15", "20", "30", "50" };
		String[] labels = { "5", "10", "15", "20", "30", "50" };
		UIOutput.make(form, "announcementsNumberDropdownLabel",
				messageLocator.getMessage("simplepage.announcements-number-dropdown-label"));
		UISelect.make(form, "announcementsNumberDropdown", options, labels, "#{simplePageBean.announcementsDropdown}",
				"5");
		UICommand.make(form, "announcements-add-item", messageLocator.getMessage("simplepage.save_message"),
	...
	private void createAddMultimediaDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "add-multimedia-dialog")
				.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.resource")));
		UILink.make(tofill, "mm-additional-instructions",
				messageLocator.getMessage("simplepage.additional-instructions-label"),
				getLocalizedURL("multimedia.html", true));
		UILink.make(tofill, "mm-additional-website-instructions",
				messageLocator.getMessage("simplepage.additional-website-instructions-label"),
		...
		UILink link = UIInternalLink.make(form, "mm-choose", messageLocator.getMessage("simplepage.choose_existing_or"),
				fileparams);
		...
		UIOutput.make(form, "mm-test-tryother").decorate(
				new UIFreeAttributeDecorator("value", messageLocator.getMessage("simplepage.mm-test-tryother")));
		UIOutput.make(form, "mm-test-start-over").decorate(
				new UIFreeAttributeDecorator("value", messageLocator.getMessage("simplepage.mm-test-start-over")));
	...
	private void createImportCcDialog(UIContainer tofill) {
		UIOutput.make(tofill, "import-cc-dialog")
				.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.import_cc")));

		UIForm form = UIForm.make(tofill, "import-cc-form");
		makeCsrf(form, "csrf11");

		UICommand.make(form, "import-cc-submit", messageLocator.getMessage("simplepage.import_message"),
		...
		if (numAssignEngines == 0) {
			...
			UISelect assignselect = UISelect.make(form, "assigntools", values.toArray(new String[1]),
	...
	private void createExportCcDialog(UIContainer tofill) {
		UIOutput.make(tofill, "export-cc-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.export-cc-title")));
		...
		UIOutput.make(form, "export-cc-v11"); // value is handled by JS, so RSF
												...
												// input
		UIOutput.make(form, "export-cc-v13"); // value is handled by JS, so RSF
												// doesn't need to treat it as
												// input
		UIOutput.make(form, "export-cc-bank"); // value is handled by JS, so RSF
												// doesn't need to treat it as
												// input
		UICommand.make(form, "export-cc-submit", messageLocator.getMessage("simplepage.exportcc-download"),
				"#{simplePageBean.importCc}");
	...
	private void createEditMultimediaDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "edit-multimedia-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.editMultimedia")));
		...
		fileparams.setResourceType(true);
		fileparams.viewID = ResourcePickerProducer.VIEW_ID;
		UIInternalLink.make(form, "change-resource-mm", messageLocator.getMessage("simplepage.change_resource"),
				fileparams);
		...
		UICommand.make(form, "edit-multimedia-item", messageLocator.getMessage("simplepage.save_message"),
				"#{simplePageBean.editMultimedia}");

		UIInput.make(form, "multimedia-item-id", "#{simplePageBean.itemId}");

		UICommand.make(form, "delete-multimedia-item", messageLocator.getMessage("simplepage.delete"),
				"#{simplePageBean.deleteItem}");
	...
	private void createYoutubeDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "youtube-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.edit_youtubelink")));
	...
	private void createMovieDialog(UIContainer tofill, SimplePage currentPage) {
		...
		fileparams.setResourceType(true);
		fileparams.viewID = ResourcePickerProducer.VIEW_ID;
		UIInternalLink.make(form, "change-resource-movie", messageLocator.getMessage("simplepage.change_resource"),
				fileparams);

		fileparams.setCaption(true);
		UIInternalLink.make(form, "change-caption-movie", messageLocator.getMessage("simplepage.change_caption"),
				fileparams);
	...
	private void createEditTitleDialog(UIContainer tofill, SimplePage page, SimplePageItem pageItem,
			ContentResource cssLink) {
		if (pageItem.getType() == SimplePageItem.STUDENT_CONTENT)
			UIOutput.make(tofill, "edit-title-dialog")
					.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.editTitle")));
		else
			UIOutput.make(tofill, "edit-title-dialog")
					.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.title")));
		...
		if (!simplePageBean.isStudentPage(page)) {
			...
			// cssLink is set above to the actual CSS resource that's active for
			// the page
			...
			UIOutput.make(form, "cssDefaultInstructions",
					messageLocator.getMessage("simplepage.css-default-instructions"));
			...
			if (showSetOwner) {
				...
				UIOutput.make(form, "ownerDefaultInstructions",
						messageLocator.getMessage("simplepage.owner-default-instructions")
								.replace("{1}", messageLocator.getMessage("simplepage.permissions"))
								.replace("{2}", messageLocator.getMessage("simplepage.more-tools")));
				...
				Set<String> siteUsersCanUpdate = simplePageBean.getCurrentSite()
						.getUsersIsAllowed(SimplePage.PERMISSION_LESSONBUILDER_UPDATE);
				possOwners.removeAll(siteUsersCanUpdate);
				if (isOwned) {
					if (possOwners.contains(owner)) {
						int i = possOwners.indexOf(owner);
						Collections.swap(possOwners, i, 0); // put owner top of
															// list
					} else {
				...
				UIOutput.make(form, "changeOwnerDropdownLabel",
						messageLocator.getMessage("simplepage.change-owner-dropdown-label"));
				UISelect.make(form, "changeOwnerDropdown", roleOptions.toArray(new String[roleOptions.size()]),
						roleLabels.toArray(new String[roleLabels.size()]), "#{simplePageBean.newOwner}", null);
			}
		}
	...
	private void createNewPageDialog(UIContainer tofill, SimplePage page, SimplePageItem pageItem) {
		UIOutput.make(tofill, "new-page-dialog")
				.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.new-page")));
	...
	private void createRemovePageDialog(UIContainer tofill, SimplePage page, SimplePageItem pageItem) {
		UIOutput.make(tofill, "remove-page-dialog")
				.decorate(new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.remove-page")));
		...
		UIComponent button = UICommand.make(form, "remove-page-submit", messageLocator.getMessage("simplepage.remove"),
				"#{simplePageBean.removePage}");
		if (!simplePageBean.isStudentPage(page)) // not student page
			button.decorate(
					new UIFreeAttributeDecorator("onclick", "window.location='/lessonbuilder-tool/removePage?site="
							+ simplePageBean.getCurrentSiteId() + "&page=" + page.getPageId() + "';return false;"));
	...
	private void createCommentsDialog(UIContainer tofill) {
		UIOutput.make(tofill, "comments-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.edit_commentslink")));
	...
	private void createStudentContentDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "student-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.edit_studentlink")));
		...
		UICommand.make(form, "delete-student-item", messageLocator.getMessage("simplepage.delete"),
				"#{simplePageBean.deleteItem}");
		UICommand.make(form, "update-student", messageLocator.getMessage("simplepage.edit"),
				"#{simplePageBean.updateStudent}");
		UICommand.make(form, "cancel-student", messageLocator.getMessage("simplepage.cancel"), null);

		// RU Rubrics
		UIOutput.make(tofill, "peer-eval-create-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.peer-eval-create-title")));
	}

	private void createQuestionDialog(UIContainer tofill, SimplePage currentPage) {
		UIOutput.make(tofill, "question-dialog").decorate(
				new UIFreeAttributeDecorator("title", messageLocator.getMessage("simplepage.edit_questionlink")));

		UIForm form = UIForm.make(tofill, "question-form");
		makeCsrf(form, "csrf21");

		UISelect questionType = UISelect.make(form, "question-select", new String[] { "multipleChoice", "shortanswer" },
		...
		UIOutput.make(form, "question-shortans-del")
				.decorate(new UIFreeAttributeDecorator("alt", messageLocator.getMessage("simplepage.delete")));
		UIOutput.make(form, "question-mc-del")
				.decorate(new UIFreeAttributeDecorator("alt", messageLocator.getMessage("simplepage.delete")));
		...
		UICommand.make(form, "delete-question-item", messageLocator.getMessage("simplepage.delete"),
				"#{simplePageBean.deleteItem}");
		UICommand.make(form, "update-question", messageLocator.getMessage("simplepage.edit"),
	...
	/**
	 * Returns a Status object with the status of a user's response to a
	 * question. For showing status images next to the question.
	 */
	...
	// /lessonbuilder-tool/template/instructions/general.html
	private boolean UrlOk(String url) {
		...
		if (url.startsWith("http:") || url.startsWith("https:")) {
			...
			try {
				...
				log.error("Internationalization url lookup timed out for " + url
	...
	private long findMostRecentComment() {
		List<SimplePageComment> comments = simplePageToolDao.findCommentsOnPageByAuthor(
				simplePageBean.getCurrentPage().getPageId(), UserDirectoryService.getCurrentUser().getId());
	...
	private void printGradingForm(UIContainer tofill) {
		...
		if (!printedGradingForm) {
			...
			Object sessionToken = SessionManager.getCurrentSession().getAttribute("sakai.csrf.token");
			UIInput csrfInput = UIInput.make(gradingForm, "csrf", "gradingBean.csrfToken",
	...
	private void makeSaveChecklistForm(UIContainer tofill) {
		...
		if (!saveChecklistFormNeeded) {
			...
			UIInput checklistIdInput = UIInput.make(saveChecklistForm, "saveChecklistForm-checklistId",
					"checklistBean.checklistId");
			UIInput checklistItemIdInput = UIInput.make(saveChecklistForm, "saveChecklistForm-checklistItemIdInput",
					"checklistBean.checklistItemId");
			UIInput checklistItemDone = UIInput.make(saveChecklistForm, "saveChecklistForm-checklistItemDone",
					"checklistBean.checklistItemDone");
			Object sessionToken = SessionManager.getCurrentSession().getAttribute("sakai.csrf.token");
			String sessionTokenString = null;
			if (sessionToken != null)
				sessionTokenString = sessionToken.toString();
			UIInput checklistCsrfInput = UIInput.make(saveChecklistForm, "saveChecklistForm-csrf",
					"checklistBean.csrfToken", sessionTokenString);

			UIInitBlock.make(tofill, "saveChecklistForm-init", "checklistDisplay.initSaveChecklistForm",
					new Object[] { checklistIdInput, checklistItemIdInput, checklistItemDone, checklistCsrfInput,
							"checklistBean.results" });
	...
	private String getItemPath(SimplePageItem i) {
		...
		// users seem to want paths for the embedded items, so they can see
		// what's going on
		if (i.getType() == SimplePageItem.MULTIMEDIA) {
			...
			if ("1".equals(mmDisplayType)) {
				...
				return FormattedText.escapeHtml(
						i.getItemURL(simplePageBean.getCurrentSiteId(), simplePageBean.getCurrentPage().getOwner()),
						false);
			}
		}
		...
		for (int tokenIndex = 3; tokenIndex < itemPathTokens.length; tokenIndex++) {
			if (isURL) {
				itemPath += "/<a target=\"_blank\" href=\"\" class=\"" + URLEncoder.encode(pathId) + "\">"
						+ FormattedText.escapeHtml(itemPathTokens[tokenIndex], false) + "</a>";
				isURL = false;
			} else
	...
	private String[] makeStudentRubric = { null, "peer-eval-row-student:", "peerReviewIdStudent",
			"peerReviewTextStudent", "peer-eval-row-data", "#{simplePageBean.rubricPeerGrade}" };
	private String[] makeMaintainRubric = { "peer-eval-title", "peer-eval-row:", "peerReviewId", "peerReviewText", null,
			null };

	private void makePeerRubric(UIContainer parent, SimplePageItem i, String[] peerReviewRsfIds,
			Map<Long, Integer> selectedCells, Map<Long, Map<Integer, Integer>> dataMap, boolean allowSubmit) {
		// log.info("makePeerRubric(): i.getAttributesString() " +
		// i.getAttributeString());
		// log.info("makePeerRubric(): i.getAttribute(\"rubricTitle\") " +
		// i.getAttribute("rubricTitle"));
		// log.info("makePeerRubric(): i.getJsonAttribute(\"rows\") " +
		// i.getJsonAttribute("rows"));
		...
		class RubricRow implements Comparable {
			...
			public RubricRow(Long id, String text) {
				this.id = id;
				this.text = text;
			}
	...
	public void setScormEntity(LessonEntity scormEntity) {
		ShowPageProducer.scormEntity = scormEntity;
	}
	// Hoctdy - Add Scorm Link - End
